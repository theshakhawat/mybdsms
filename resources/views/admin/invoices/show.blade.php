...
@section('title', 'Invoice #' . $invoice->invoice_number . ' - MyBDSMS Admin')
    ...
    <div class="col-lg-9">
        ...
        <div class="card border-0 shadow-sm rounded-4 p-3 bg-white mb-4 d-print-none">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                <a href="{{ route('admin.invoices.index') }}" class="btn btn-outline-secondary rounded-pill px-3">
                    <i class="fas fa-arrow-left me-1"></i> Back to Invoices
                </a>
                <div class="d-flex gap-2 align-items-center">
                    <!-- Status Control -->
                    <div class="dropdown">
                        <button class="btn dropdown-toggle rounded-pill px-3 fw-semibold {{ $invoice->status === 'paid' ? 'btn-success' : ($invoice->status === 'unpaid' ? 'btn-warning text-dark' : 'btn-secondary') }}" type="button" data-bs-toggle="dropdown">
                            Status: {{ strtoupper($invoice->status) }}
                        </button>
                        <ul class="dropdown-menu shadow border-0">
                            <li>
                                <form method="POST" action="{{ route('admin.invoices.update_status', $invoice->id) }}">
                                    @csrf
                                    @method('PATCH')
                                    <input type="hidden" name="status" value="paid">
                                    <button type="submit" class="dropdown-item text-success {{ $invoice->status === 'paid' ? 'active disabled' : '' }}">
                            ...
                            <li>
                                <form method="POST" action="{{ route('admin.invoices.update_status', $invoice->id) }}">
                                    ...
                                    <button type="submit" class="dropdown-item text-warning {{ $invoice->status === 'unpaid' ? 'active disabled' : '' }}">
                                        <i class="fas fa-clock me-1"></i> Mark as Unpaid
                                    </button>
                                </form>
                            </li>
                            <li>
                                <form method="POST" action="{{ route('admin.invoices.update_status', $invoice->id) }}">
                                    ...
                                    <button type="submit" class="dropdown-item text-danger {{ $invoice->status === 'cancelled' ? 'active disabled' : '' }}">
        ...
        <div class="card border-0 shadow-sm rounded-4 p-4 p-md-5 bg-white invoice-box position-relative">
            ...
            @if($invoice->slip_image)
                <div class="border rounded-3 p-3 bg-light mb-4 small">
                    <h6 class="fw-bold text-dark mb-2"><i class="fas fa-paperclip me-1"></i> Customer Payment Slip:</h6>
                    <div class="d-flex align-items-center gap-3">
                        <a href="{{ asset($invoice->slip_image) }}" target="_blank">
                            <img src="{{ asset($invoice->slip_image) }}" alt="Payment Slip" class="rounded border shadow-sm" style="max-height: 140px; max-width: 250px; object-fit: contain; background: #fff;">
                        </a>
                        <div>
                            <a href="{{ asset($invoice->slip_image) }}" target="_blank" class="btn btn-sm btn-outline-primary rounded-pill px-3">
                                <i class="fas fa-external-link-alt me-1"></i> Open Original Slip
                            </a>
                        </div>
                    </div>
                </div>
            ...
            <div class="border-top pt-3 mt-4 text-center text-muted small">
                <p class="mb-0">MyBDSMS Admin Billing System</p>
            </div>
        </div>
    </div>
</div>
