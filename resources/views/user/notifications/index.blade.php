...
@section('title', 'Notifications - MyBDSMS')
...
<div class="card border-0 shadow-sm rounded-4 p-4 bg-white mb-4">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div>
            ...
            <p class="text-muted mb-0">Updates on your packages, invoices, and new plans</p>
        </div>
        @if($unreadCount > 0)
            <form method="POST" action="{{ route('user.notifications.mark_all_read') }}">
                @csrf
...
<div class="card border-0 shadow-sm rounded-4 p-3 bg-white mb-4">
    <div class="d-flex gap-2 flex-wrap align-items-center">
        <a href="{{ route('user.notifications.index') }}" class="btn btn-sm {{ !request('status') && !request('type') ? 'btn-dark' : 'btn-outline-secondary' }} rounded-pill px-3">
            All
        </a>
        <a href="{{ route('user.notifications.index', ['status' => 'unread']) }}" class="btn btn-sm {{ request('status') === 'unread' ? 'btn-danger' : 'btn-outline-danger' }} rounded-pill px-3">
            Unread ({{ $unreadCount }})
        </a>
        <a href="{{ route('user.notifications.index', ['type' => 'plan']) }}" class="btn btn-sm {{ request('type') === 'plan' ? 'btn-success' : 'btn-outline-success' }} rounded-pill px-3">
            <i class="fas fa-tags me-1"></i> New Plans
        </a>
        <a href="{{ route('user.notifications.index', ['type' => 'invoice']) }}" class="btn btn-sm {{ request('type') === 'invoice' ? 'btn-warning text-dark' : 'btn-outline-warning' }} rounded-pill px-3">
                ...
                <div class="flex-grow-1">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        @if(!empty($notification->action_url))
                            <a href="{{ route('user.notifications.read', $notification->id) }}" class="btn btn-sm btn-primary rounded-pill px-3 py-1">
